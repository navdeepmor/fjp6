...
function Ue3() {
    ...
    useEffect(() => {                                                                                               // Variation 3:  useEffect(() => { }, [ varName ])    here it is runs for only
        ...
        document.title = `Button clicked ${ count } times`;                                                         //                                                      - componentDisUpdate  ( which depends upon the varName defined in optional list arg )                                                 
    ...
    let changeText = (e) => {
        ...
        // console.log(txt.msg);
        setText({...txt});                                                                                          // here txt gets new address as we define it in new obj & spread value of txt in it thus react knows that it need to re-render. (txt Adrress is in stack which ref. to it's objectin heap. React only keep eye on address in stack)
    }
